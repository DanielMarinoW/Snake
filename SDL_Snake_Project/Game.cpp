...
bool Game::initSDL() {
    ...
    if (gRenderer != 0)
    {
        ...
        TextureManager::Instance()->loadTexture("image/Play.png", "play", gRenderer);
        TextureManager::Instance()->loadTexture("image/GameOver.png", "gameover", gRenderer);
        TextureManager::Instance()->loadTexture("image/snake.b.jpg", "background", gRenderer);
        TextureManager::Instance()->loadTexture("image/text.png", "text", gRenderer);
        SoundManager::Instance()->load("snake.sound/button.c.wav", "button.c", 0);
    }
    ...
    while (SDL_PollEvent(&e) != 0) {
        ...
        case SDL_MOUSEBUTTONDOWN:
            ...
            if (Play.isClicked(mouseX, mouseY)) {
                cout << "PlAY Button is c: " << endl;
                SoundManager::Instance()->ClickButton();
                Play.setState(UNKNOWN);
                startPlay = true;
                initGame();
...
void Game::render()
{
    ...
    TextureManager::Instance()->drawTexture("background", 0, 0, 800, 800, gRenderer);
    if (startPlay == true) {
    ...
    if (startPlay == false && Play.getState() == GameState::START) {
        TextureManager::Instance()->drawTexture("play", 200, 100, 400, 400, gRenderer);
    }
    if (snakeDie() || snakeDieInWall()) {
        TextureManager::Instance()->drawTexture("gameover", 130, 100, 600, 309, gRenderer); 
    }
...
bool Game::snakeDieInWall()
{
    ...
    if (newHead.x < 0 || newHead.y < 0 || newHead.x  >= SCREEN_WIDTH || newHead.y  >= SCREEN_HEIGHT) {
